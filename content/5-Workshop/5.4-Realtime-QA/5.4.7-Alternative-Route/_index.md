...
{{% notice note %}}
The `_handle_upload(event)` function located in `handler.py:272–340` handles the `POST /documents` endpoint allowing the user interface (Browser UI) to upload document files directly to the S3 bucket.
{{% /notice %}}

1. Design Trade-offs
   Rather than issuing a Presigned S3 URL for the browser to upload directly to S3, the system routes data through API Gateway + Lambda:
   - Trade-off: Incurs a slight payload transfer overhead (API Gateway caps request bodies at 10MB and base64 encoding inflates payload size by ~33%).
   - Benefits:
     - No need to enable wide CORS configurations on the `raw_documents` S3 bucket.
     - Enhanced Security: Avoids creating a Presigned-URL generation endpoint that could leak S3 write permissions to the client side.

2. Two Accepted Payload Formats
   - `content_base64` (Preferred for binary files: PDF, PNG, JPG, TIFF):
     - Base64 encodes the file's raw byte stream.
     - Ensures PDF/Image files do not get corrupted by browser attempts to decode UTF-8 strings, preserving intact byte streams to pass to AWS Textract OCR processing in Stream 1.
   - `content` (For plain text files: TXT, MD, CSV, JSON):
     - Contains raw text content or text copied-and-pasted directly from the UI interface.
...
- [5.4.8 - End-to-End Testing](../5.4.8-End-To-End-Testing/)
