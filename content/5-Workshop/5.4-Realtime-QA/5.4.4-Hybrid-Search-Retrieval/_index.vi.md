---
title: "Hybrid Search và Retrieval"
date: 2024-01-01
weight: 4
chapter: false
pre: " <b> 5.4.4 </b> "
...
Tiếp theo bước cache lookup và query rewriting ở trang [5.4.3](../5.4.3-Cache-Lookup-Query-Rewriting/), trang này trình bày phần cốt lõi của RAG: kiểm duyệt câu hỏi, tìm kiếm ngữ cảnh liên quan, và lấy đầy đủ nội dung ngữ cảnh.

#### Bước 3 — Guardrail đầu vào/ra
...
Câu hỏi gốc (sau khi đã rewrite nếu cần) được kiểm tra qua Bedrock Guardrail (đã cấu hình ở trang [5.4.2](../5.4.2-Cache-Guardrails-IAM/)) **trước khi** đưa vào bước embedding/retrieval — chặn sớm các câu hỏi vi phạm chính sách nội dung ngay từ đầu pipeline.

#### Bước 4 — Embed câu hỏi và Hybrid Search
...
{{% notice note %}}
Thay vì gọi 1 engine tìm kiếm chuyên dụng, hàm `retrieval.hybrid_search` tự quét bảng `child_table`, tính **cosine similarity** (độ tương đồng ngữ nghĩa qua vector) và **BM25 score** (độ khớp từ khóa) cho từng chunk, rồi hợp nhất 2 kết quả xếp hạng bằng thuật toán **Reciprocal Rank Fusion (RRF)** — cho ra 1 danh sách kết quả cân bằng giữa "hiểu ý" và "khớp từ khóa đúng".
{{% /notice %}}

#### Bước 5 — Lấy Parent Chunk làm ngữ cảnh cho LLM

##### 1. Hàm `fetch_parent_context` (`retrieval.py:156-189`)

Thực hiện gom danh sách parent_id duy nhất từ kết quả hybrid search (tối đa max_parents = 4) và dùng batch_get_item gọi DynamoDB lấy toàn bộ nội dung chunk cha:
...
##### 2. Luồng gọi và xử lý khi không có ngữ cảnh trong `handler.py:545-569`

Trong `handler.py`, hàm trên được gọi và kiểm tra: nếu parents rỗng thì trả về thông báo lỗi ngay lập tức mà bỏ qua bước sinh câu trả lời bằng Claude (Bedrock) — tránh lãng phí token gọi model cho một câu hỏi chắc chắn không có dữ liệu để trả lời đúng.
...
- [5.4.5 - Sinh câu trả lời và lưu lịch sử](../5.4.5-Answer-Generation-History-Storage/)
